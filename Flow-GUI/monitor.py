...
class MonitorRuns(MonitorBase):
    def __init__(self):
        ...
        terminal_layout = QVBoxLayout(self.terminal_area)
        ...
        # 创建终端进程
        self.terminal_process = QtCore.QProcess()
        self.terminal_process.setProcessChannelMode(QtCore.QProcess.MergedChannels)
        ...
        # 添加标签页
        self.bottom_tab.addTab(self.log_area, "LOG CONSOLE")
        self.bottom_tab.addTab(self.terminal_area, "TERMINAL")
        ...
        self.splitter.setStretchFactor(1, 3)  # 底部面板占30%
        
        # 将分隔器添加到主布局
        main_layout.addWidget(self.splitter)
        
        # 打印欢迎信息
        self.log_message("Welcome to XMeta.", "info")
        
        # 连接终端标签页切换信号
        self.bottom_tab.currentChanged.connect(self.on_bottom_tab_changed)
        
    def on_bottom_tab_changed(self, index):
        """处理底部标签页切换事件"""
        if index == 1:  # TERMINAL标签页
            # 如果终端进程不在运行，启动它
            if self.terminal_process.state() == QtCore.QProcess.NotRunning:
                self.start_terminal()
    ...
    def start_terminal(self):
        ...
        if self.terminal_process.state() == QtCore.QProcess.NotRunning:
            # 设置工作目录
            self.terminal_process.setWorkingDirectory(self.combo_sel)
            ...
            if sys.platform == 'darwin':  # macOS
                self.terminal_process.start('open', ['-a', 'Terminal', self.combo_sel])
            else:  # Linux/Unix
                self.terminal_process.start('gnome-terminal', ['--working-directory=' + self.combo_sel])
    ...
    def handle_terminal_error(self):
        ...
        text = bytes(data).decode('utf-8')
        self.terminal_text.setTextColor(QtGui.QColor("#FF6B68"))
        self.terminal_text.append(text)
        self.terminal_text.setTextColor(QtGui.QColor("#FFFFFF"))
    ...
    def click_event(self):
        ...
        # 如果当前是TERMINAL标签页，重启终端进程
        if self.bottom_tab.currentIndex() == 1:
            self.terminal_process.kill()  # 结束当前终端进程
            self.start_terminal()         # 启动新的终端进程
